...
import '../widgets/account_ledger_add_dialog.dart';
...
class AccountLedgerScreenState extends State<AccountLedgerScreen> {
  ...
  Future<void> _openAddDialog() async {
    final result = await showDialog<CreateAccountLedgerRequest>(
      context: context,
      builder: (_) => const AccountLedgerAddDialog(),
    );
    if (result == null || !mounted) return;
    await context.read<AccountLedgerViewModel>().addAccountLedger(result);
  }
                  ...
                  Expanded(
                    child: TextField(
                      ...
                      decoration: const InputDecoration(
                        hintText: 'Search by name, alias, group or ID...',
                            ...
                            : ListView.separated(
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  return AccountLedgerCard(
                                    ledger: ledger,
                                    groupName: viewModel.groupNameOf(ledger),
...
/// Table view of account ledgers for wide screens.
            ...
            for (final ledger in ledgers)
              DataRow(
                cells: [
                  ...
                  DataCell(Text(ledger.alias ?? '—')),
                  DataCell(Text(groupName(ledger))),
                  DataCell(
                    Text(
                      ledger.description ?? '—',
                      maxLines: 1,
                  ...
                  DataCell(_StatusBadge(isActive: ledger.isActive)),
                  DataCell(
                    IconButton(
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text(
            'Try a different search or add a new ledger.',
            style: theme.textTheme.bodyMedium?.copyWith(
              color: AppColors.textSecondary,
            ),
          ),
